  ...
  private RemoteViews buildUpdate(Context ctxt, int[] appWidgetIds) {
    ...
    PendingIntent pi=PendingIntent.getBroadcast(ctxt, 0 , i,
    	...
    	if(suma != null){
		    ...
		    minuty = (sumaInt / 60);
		    sekundy = (sumaInt % 60);
        	updateViews.setTextViewText(R.id.textView1, minuty + " min. " + sekundy +" sek.");
    	}
    	else{
        	updateViews.setTextViewText(R.id.textView1, "0" + " min. " + "0"+" sek.");
    	}
	...
	if(sumaPrzych != null){
	    ...
	    minuty = (sumaInt / 60);
	    sekundy = (sumaInt % 60);
	    updateViews.setTextViewText(R.id.wych_pods,minuty + " min. " + sekundy +" sek.");
	}
	else{
		updateViews.setTextViewText(R.id.wych_pods, "0" + " min. " + "0"+" sek.");
	}
...
//	  myDb = dataEvent.getReadableDatabase();
//      return myDb.rawQuery("select SUM(czas) from wychodzace_polaczenia", null);
//  }	  
